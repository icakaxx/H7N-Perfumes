...
export function Footer({ language, messages }: FooterProps) {
  ...
  return (
    <footer className="mt-12 border-t border-gray-200 pt-4 text-[11px] text-slate-500">
      <p className="text-xs text-slate-700">{messages.footer.limitedOffer}</p>
      <div className="mt-3 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex flex-wrap items-center gap-3">
          <span>{messages.footer.rights}</span>
          <nav className="flex gap-3 text-[11px] text-slate-500">
